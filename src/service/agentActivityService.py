...
from dal.db import gtAgentActivityManager, atomic_transaction
...
async def update_activity_progress(
    ...
    *,
    ...
    # LLM_INFER 成功完成时推送 USAGE_UPDATED 事件，供前端实时统计
    if (
        status == AgentActivityStatus.SUCCEEDED
        and activity is not None
        and activity.activity_type == AgentActivityType.LLM_INFER
    ):
        meta = activity.metadata or {}
        prompt = meta.get("final_prompt_tokens") or meta.get("prompt_tokens") or 0
        completion = meta.get("final_completion_tokens") or meta.get("completion_tokens") or 0
        total = meta.get("final_total_tokens") or meta.get("total_tokens") or (prompt + completion)
...
async def fail_started_activities(agent_id: int, error_message: str = "cancelled by user") -> list[GtAgentActivity]:
    """将某个 Agent 当前仍处于 STARTED 的活动统一标记为 FAILED（事务内批量更新）。"""

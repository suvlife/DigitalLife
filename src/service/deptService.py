...
# per-team 部门树缓存，避免高频全表扫描+建树
_dept_tree_cache: Dict[int, GtDept] = {}


def invalidate_dept_tree_cache(team_id: int) -> None:
    """失效指定团队的部门树缓存。所有写入部门路径须调用。"""
    _dept_tree_cache.pop(team_id, None)
...
async def overwrite_dept_tree(team_id: int, root: GtDept) -> None:
    ...
    invalidate_dept_tree_cache(team_id)
...
async def get_dept_tree(team_id: int) -> GtDept | None:
    ...
    带 per-team 缓存，返回深拷贝防止调用方修改污染缓存。
    写入路径（upsert_dept/delete_dept/overwrite_dept_tree）须调 invalidate_dept_tree_cache。
    """
    cached = _dept_tree_cache.get(team_id)

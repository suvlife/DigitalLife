...
async def update_task(
    ...
    if new_status == TaskStatus.DONE:
        ...
        # 普通协作子任务 DONE 不发布博客。只有统一 final conclusion 入口可入队，
        # 防止每个子任务和 submit_conclusion 产生重复/碎片文章。
...
async def _unblock_pending_dependents(completed_task: GtAgentTask) -> None:
    """查找依赖 completed_task 且所有依赖均已 DONE 的 PENDING 任务，自动转为 TODO。"""
    try:
        # 查找同团队下 PENDING 状态、depends_on 包含 completed_task.id 的任务
        team_tasks = await gtAgentTaskManager.list_tasks(
            team_id=completed_task.team_id,
            status=TaskStatus.PENDING,
            limit=100,
        )
        for t in team_tasks:
            if not t.depends_on or completed_task.id not in t.depends_on:
                continue
            # 检查所有依赖是否都已 DONE
            ...
            if all(d.status == TaskStatus.DONE for d in dep_tasks):
                ...
                from service import messageBus as _mb
                from constants import MessageBusTopic as _MBT
                _mb.publish(_MBT.TASK_CHANGED, task=t, old_status=TaskStatus.PENDING.value)
    except Exception:
        logger.warning("自动唤醒下游 PENDING 任务失败: completed_task_id=%d", completed_task.id, exc_info=True)

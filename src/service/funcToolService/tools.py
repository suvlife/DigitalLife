...
2. 如果你确认不需要发言，请设置 confirm_no_need_talk=true 重新调用 finish_action。""",
        ...
        # Agent 本轮有发言时，触发 Ghost 博客自动发布（每个专家各发一篇）
        if _context.chat_room.current_turn_has_content:
            try:
                import asyncio as _aio
                from service import ghostService
                from model.dbModel.gtAgent import GtAgent as _GtAgent

                agent = await _GtAgent.aio_get_or_none(_GtAgent.id == _context.agent_id)
                agent_name = agent.display_name if agent else f"专家{_context.agent_id}"

                class _AgentBlogProxy:
                    def __init__(self, room, agent_name, agent_id):
                        self.title = f"{agent_name}：{room.name} 分析"
                        self.description = room.initial_topic or ""
                        self.result = ""
                        self.room_id = room.room_id
                        self._filter_agent_id = agent_id  # 只发布该 Agent 的消息

                proxy = _AgentBlogProxy(_context.chat_room, agent_name, _context.agent_id)
                _aio.create_task(ghostService.publish_task_if_enabled(proxy))
                logger.info("Agent 发言完成，触发 Ghost 博客发布: agent=%s, room=%s", agent_name, _context.chat_room.name)
            except Exception as e:
                logger.warning("Ghost 博客发布触发失败（不影响行动）: %s", e)

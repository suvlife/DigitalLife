...
class ChatRoom:
    ...
    @property
    def scheduler(self) -> RoomScheduler:
        """暴露调度器供工具（如 submit_conclusion）访问。"""
    ...
    async def activate_scheduling(self) -> bool:
        ...
        if not self.messages:
            ...
            msg = GtRoomMessage(room_id=self.room_id, sender_id=self.SYSTEM_MEMBER_ID,
                                content=content, send_time=datetime.now(timezone.utc))
    ...
    async def _append_message(
        ...
        message = GtRoomMessage(room_id=self.room_id, sender_id=sender_id,
                                content=content, send_time=send_time or datetime.now(timezone.utc),

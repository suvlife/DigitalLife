...
async def create_team(
    ...
    if config and config.get("working_directory"):
        ...
        if workspace_root:
            fileUtil.assert_path_within_sandbox(workdir, workspace_root)
...
async def update_team_base_info(team_id: int, working_directory: str | None = None, config_updates: dict | None = None) -> GtTeam:
    ...
    if working_directory is not None:
        if working_directory:
            ...
            workspace_root = configUtil.get_app_config().setting.workspace_root
            if workspace_root:
                fileUtil.assert_path_within_sandbox(working_directory, workspace_root)

...
# 序列化 _agents 注册表的变更，避免 load/unload/hot_reload 跨 await 交错
# 导致并发 get_agent 拿到已关闭实例或注册表双字典不同步。
...
async def hot_reload_agent(agent_id: int) -> None:
    ...
    整个流程在 _agents_lock 内执行，避免并发 hot_reload/get_agent 拿到
    已关闭实例或注册表在替换瞬间不一致。
    """
    async with _agents_lock:
        await _hot_reload_agent_locked(agent_id)


async def _hot_reload_agent_locked(agent_id: int) -> None:
    """hot_reload_agent 的锁内实现。"""

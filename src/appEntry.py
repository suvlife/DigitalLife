...
def _show_port_in_use_dialog(port: int) -> None:
    ...
    # 转义 AppleScript 字符串特殊字符，防止 i18n 文案含引号/反斜杠时注入
    msg_escaped = msg.replace("\\", "\\\\").replace('"', '\\"') if msg else ""
    if sys.platform == "darwin":
        subprocess.run(
            ["osascript", "-e", f'display alert "{msg_escaped}" as critical'],
            capture_output=True,
        )
    else:

...
def _escape_applescript_string(text: str) -> str:
    """转义 AppleScript 字符串字面量中的特殊字符，防止命令注入。

    i18n 文案可能含双引号或反斜杠，直接拼入 osascript 会破坏语法或被注入。
    """
    if not text:
        return ""
    # AppleScript 字符串中需转义反斜杠和双引号
    ...
    def _cb_reset_data(self, icon, item) -> None:
        ...
        if sys.platform == "darwin":
            result = subprocess.run(
                [
                    "osascript", "-e",
                    f'display dialog "{_escape_applescript_string(i18nUtil.t("confirm_reset"))}" buttons {{\"取消\", \"确认\"}} default button \"取消\" with icon caution'
                ],
        ...
        # 注：托盘菜单回调在 pystray 独立线程执行，不阻塞 asyncio 事件循环。
        import time
        time.sleep(0.5)
        ...
        if sys.platform == "darwin":
            subprocess.run(
                [
                    "osascript", "-e",
                    f'display notification "{_escape_applescript_string(i18nUtil.t("reset_done_body"))}" with title "{_escape_applescript_string(i18nUtil.t("reset_done_title"))}"'
                ],
                capture_output=True
            )
        else:
        ...
        # 正常退出以触发 atexit/finally 清理（DB flush、连接关闭等）。
        # 此前使用 os._exit(0) 会跳过所有清理，可能导致 SQLite 写未刷盘。
        sys.exit(0)

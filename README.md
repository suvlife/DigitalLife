...
- **三栏沉浸式讨论工作台**：V2 与 V3 房间页升级为“讨论信息 / 大对话区 / 运行状态”三栏布局，显著扩大专家发言显示空间；紧凑字号与自适应响应式布局可在单屏展示更多内容，减少频繁滚动。
- **讨论组默认全员参与**：新建各院研究室或讨论组时默认勾选当前全部专家，仍可按需取消个别成员，降低漏选专家导致讨论不完整的概率。
- **性能与稳定性审计整改**：用量统计下推 SQLite 聚合、房间权限批量校验、WebSocket 序列化复用、有界 LRU 缓存、Office 文件异步解析与 20 MB 限制，并补充 session 清理和并发注册保护。
- **安全部署强化**：systemd 部署默认使用专用非 root 用户，减少服务进程与数据目录权限暴露。

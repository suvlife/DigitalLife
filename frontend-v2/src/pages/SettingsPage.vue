...
const ghost = reactive<api.GhostConfig>({ enabled: false, api_url: '', admin_api_key: '', content_api_key: '', auto_publish: true, publish_status: 'published', has_admin_key: false, has_content_key: false });
...
async function testGhost() { saving.value = true; clearFeedback(); ghostTestResult.value = ''; try { const result = await api.testGhostConfig({ api_url: ghost.api_url.trim() || undefined, admin_api_key: ghostAdminKey.value || undefined }); ghostTestResult.value = `${result.message}${result.site_title ? ` · ${result.site_title}` : ''}`; } catch (e) { fail(e, 'Ghost 测试失败'); } finally { saving.value = false; } }

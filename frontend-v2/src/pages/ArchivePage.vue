...
<template>
  <div class="page archive-page">
    ...
    <section v-else-if="pagedRuns.length" class="archive-list" aria-label="历史 Run 列表">
      <RouterLink v-for="run in pagedRuns" :key="run.id" class="archive-card panel" :to="{ name: 'run', params: { teamId: run.teamId, runId: run.id } }">

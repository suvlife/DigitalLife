...
export interface Team { id: number; name: string; i18n: I18n; enabled: boolean; config: Record<string, unknown>; createdAt?: string; updatedAt?: string }
...
export interface Room { id: number; teamId: number; name: string; i18n: I18n; type: 'group'|'private'; state: string; needScheduling: boolean; currentTurnAgentId: number|null; agentIds: readonly number[]; tags: readonly string[]; initialTopic?: string|null }
...
export interface RunSnapshot { id:string; teamId:number; phase:RunPhase; progress:number; question:string; finalAnswer:string; roomRuns:Record<number, RoomRuntime>; activeAgentIds:readonly number[]; completedRoomIds:readonly number[]; publication:Publication; startedAt?:string; finishedAt?:string; source:'native'|'fallback' }
...
export type NormalizedEvent =
 ...
 | { type:'team_reloaded'; teamId:number };

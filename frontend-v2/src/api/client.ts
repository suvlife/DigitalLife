...
function message(x:any,roomId:number):Message{return{id:x.db_id!=null?Number(x.db_id):(x.id!=null?Number(x.id):null),roomId,senderId:Number(x.sender_id),senderName:String(x.sender_display_name||''),content:String(x.content||''),sentAt:String(x.send_time||x.time||''),seq:x.seq==null?null:Number(x.seq),immediate:Boolean(x.insert_immediately)}}

...
async def test_infer_retries_with_exponential_backoff_until_success(monkeypatch):
    ...
    async def _fake_send_request_non_stream(request, url, api_key, custom_llm_provider=None, extra_headers=None, request_id=""):
        attempts["count"] += 1
        if attempts["count"] < 4:
            raise InternalServerError(f"temporary failure {attempts['count']}", "model", 500)
...
async def test_infer_stream_retries_up_to_limit_then_returns_failure(monkeypatch):
    status_events: list[llmService.InferRequestStatusEvent] = []
    sleep_mock = AsyncMock()
    fake_send_request_stream = AsyncMock(side_effect=InternalServerError("stream temporary failure", "model", 500))
...
def test_classify_unknown_error_is_retryable():
    ...
    # 安全修复：UNKNOWN 是兜底分类，可能含编程错误，不再作为可重试分类，
    # 避免确定性失败被重试 8 次（126 秒）。
    assert LlmErrorCategory.UNKNOWN not in RETRYABLE_CATEGORIES
...
def test_classify_context_window_keyword():
    ...
    # 收紧关键词：移除过宽的 "max_tokens"/"token limit"（输出超限/参数错误也会含这些词）
    ...
    # 这两个关键词不再判为 context overflow
    for kw in ("max_tokens", "token limit"):
        result = classify_llm_error(RuntimeError(kw))
        assert result != LlmErrorCategory.CONTEXT_WINDOW, f"keyword '{kw}' should NOT be CONTEXT_WINDOW"

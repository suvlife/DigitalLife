...
class TestDetectRunEnv:
    def test_source_by_default(self, monkeypatch):
        monkeypatch.delenv("DIGITALLIFE_RUN_ENV", raising=False)
        assert backend_main._detect_run_env() == "source"

    def test_docker_when_env_var_set(self, monkeypatch):
        monkeypatch.setenv("DIGITALLIFE_RUN_ENV", "docker")
...
async def test_main_waits_for_shutdown_event_and_runs_cleanup(monkeypatch):
    ...
    monkeypatch.setattr(backend_main.runService, "startup", AsyncMock())
    monkeypatch.setattr(backend_main.ghostService, "startup", AsyncMock())
    ...
    monkeypatch.setattr(backend_main.runService, "shutdown", MagicMock())
    monkeypatch.setattr(backend_main.ghostService, "shutdown", AsyncMock())
